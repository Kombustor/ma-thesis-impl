...
export const whitelistRouter = createProtectedRouter()
  ...
  .mutation('create', {
    ...
    async resolve({ input, ctx }) {
      return await ctx.prisma.emailWhitelist.create({ data: input.data });
    },
  })
  .mutation('update', {
    input: UpdateEmailWhitelistSchema,
    async resolve({ input, ctx }) {
      return await ctx.prisma.emailWhitelist.update({
        where: { id: input.id },
        data: input.data,
      });
    },
  ...
  .mutation('delete', {
    input: EmailWhitelistId,
    async resolve({ input, ctx }) {
      return await ctx.prisma.emailWhitelist.delete({ where: { id: input } });
    },
  });
